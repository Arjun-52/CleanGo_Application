...
class BookingProvider with ChangeNotifier {
  ...
  double get cartTotal => _cartItems.fold(0, (sum, item) => sum + (item.totalPrice ?? 0));
  ...
  void addToCart(BookingItem item) {
    final existingIndex = _cartItems.indexWhere((i) => i.serviceId == item.serviceId);
    if (existingIndex >= 0) {
      ...
      _cartItems[existingIndex] = BookingItem(
        ...
        totalPrice: (existing.pricePerUnit ?? 0) * (existing.quantity + item.quantity),
      );
    } else {
      _cartItems.add(item);
    }
